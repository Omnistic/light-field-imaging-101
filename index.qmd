...
The threshold value was manually selected to ensure the number of detected centroids matches the total lenslet count (2&thinsp;729) as described in @sec-mla.
...
The next step is to reshape the 2D light-field image into a 4D light-field array as a function of U and V (the spatial coordinates), and S and T (the angular coordinates). Each centroid location from @sec-calibration has coordinates corresponding to a position in the S and T space. From this position, a U and V map is extracted from the light-field image for each centroid. An interpolation step is introduced to have the centroid aligned on the pixel grid. A masking operation is performed to only keep the circular angular information from the corresponding lenslet.
...
mask = old_u_grid**2 + old_v_grid**2 <= 17**2
